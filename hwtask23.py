...
list_of_n = [-n+i for i in range (n*2+1)]
print (list_of_n)
with open ("positions.txt") as indexes:
    ind = list(map(int, indexes.readlines()))
    print (f'{ind} - list of indexes from file positions.txt')
    product1 = prod ([list_of_n [i] for i in ind])
    print (f'product of numbers with indexes from file positions.txt equals {product1}')
